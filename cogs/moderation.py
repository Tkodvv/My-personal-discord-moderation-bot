...
class ModerationCog(commands.Cog):
    ...
    def _dyno_line(
        ...
        *,
        ...
        verb: str,                    # "banned", "kicked", "timed out", "unbanned"
        member: discord.abc.User,
        case_id: int,
        reason: Optional[str] = None,
        extra_right: Optional[str] = None,   # e.g. "DM Sent: Yes", "Duration: 30m"
    ) -> discord.Embed:
        """Compact, Dyno-style line with avatar as author icon + footer details."""
        e = discord.Embed(color=self._brand_color(), timestamp=discord.utils.utcnow())
        # Author line looks like: "SomeUser#1234 was banned."
        e.set_author(name=f"{member} was {verb}.", icon_url=member.display_avatar.url)
        # Make a small, single-line-ish description with optional reason + extras
        bits = []
        if reason:
            # keep it short if someone pastes an essay
            r = reason.strip()
            if len(r) > 300:
                r = r[:297] + "..."
            bits.append(f"*Reason:* {r}")
        if extra_right:
            bits.append(extra_right)
        if bits:
            e.description = " • ".join(bits)

        # Footer: Case + User ID (Dyno vibe)
        footer = f"Case #{case_id} • User ID: {member.id}"
        e.set_footer(text=footer)

        # Keep it tiny: no thumbnail, no big fields.
        # Optional small server flair in the header (not necessary for size)
        if guild and guild.icon:
            # (We don't set embed.thumbnail to avoid making it tall)
            pass
        return e
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = "No reason provided"):
        ...
        try:
            ...
            embed = self._dyno_line(
                guild=interaction.guild,
                verb="kicked",
                member=member,
                case_id=case_id,
                reason=reason,
                extra_right=f"DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}",
            )
    ...
    async def ban(
        ...
        try:
            ...
            if delete_messages and delete_messages > 0:
                extra.append(f"Msgs Deleted: {delete_messages}d")
            extra.append(f"DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}")
            embed = self._dyno_line(
                guild=interaction.guild,
                verb="banned",
                member=member,
    ...
    async def timeout(self, interaction: discord.Interaction, member: discord.Member, duration: int, reason: Optional[str] = "No reason provided"):
        ...
        try:
            ...
            self._store_case(
                ...
                moderator_id=interaction.user.id,
                reason=reason,
                extra={"dm_sent": dm_sent, "minutes": duration, "until": int(until.timestamp())},
            )
            ...
            embed = self._dyno_line(
                ...
                case_id=case_id,
                reason=reason,
                extra_right=f"Duration: {duration}m • DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}",
            )
            await interaction.response.send_message(embed=embed)
    ...
    async def reason_cmd(self, interaction: discord.Interaction, case_id: int, reason: str):
        ...
        # Basic permission gate similar to Dyno (common mod perms)
        ...
        e.set_footer(text=f"Case #{case_id} • User ID: {target_id}")
        await interaction.response.send_message(embed=e)

    # --------------------
    # Prefix commands (same compact look)
    # --------------------
    ...
    async def prefix_kick(self, ctx, member: discord.Member, *, reason="No reason provided"):
        ...
        if not has_moderation_permissions(ctx.author, member):
            await ctx.send("❌ You don't have permission to kick this member.", delete_after=5); return
        if not has_higher_role(ctx.guild.me, member):
            await ctx.send("❌ I cannot kick this member due to role hierarchy.", delete_after=5); return

        case_id = self._next_case_id(ctx.guild.id)
        dm_sent = False
        try:
            dm = discord.Embed(title=f"You were kicked from {ctx.guild.name}", description=f"**Reason:** {reason}", color=discord.Color.orange(), timestamp=discord.utils.utcnow())
            if ctx.guild.icon: dm.set_thumbnail(url=ctx.guild.icon.url)
            await member.send(embed=dm); dm_sent = True
        except Exception: dm_sent = False

        try:
            await member.kick(reason=f"Kicked by staff: {reason}")
            self._store_case(ctx.guild.id, case_id, action="Kick", target_id=member.id, target_tag=f"{member}", moderator_id=ctx.author.id, reason=reason, extra={"dm_sent": dm_sent})
            embed = self._dyno_line(guild=ctx.guild, verb="kicked", member=member, case_id=case_id, reason=reason, extra_right=f"DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}")
    ...
    async def prefix_ban(self, ctx, member: discord.Member, *, reason="No reason provided"):
        await self.delete_command_message(ctx)
        if not isinstance(ctx.author, discord.Member) or not ctx.guild: return
        if not has_moderation_permissions(ctx.author, member):
            await ctx.send("❌ You don't have permission to ban this member.", delete_after=5); return
        if not has_higher_role(ctx.guild.me, member):
            await ctx.send("❌ I cannot ban this member due to role hierarchy.", delete_after=5); return

        case_id = self._next_case_id(ctx.guild.id)
        dm_sent = False
        try:
            dm = discord.Embed(title=f"You were banned from {ctx.guild.name}", description=f"**Reason:** {reason}", color=discord.Color.red(), timestamp=discord.utils.utcnow())
            if ctx.guild.icon: dm.set_thumbnail(url=ctx.guild.icon.url)
            await member.send(embed=dm); dm_sent = True
        except Exception: dm_sent = False
        ...
        try:
            ...
            self._store_case(ctx.guild.id, case_id, action="Ban", target_id=member.id, target_tag=f"{member}", moderator_id=ctx.author.id, reason=reason, extra={"dm_sent": dm_sent})
            embed = self._dyno_line(guild=ctx.guild, verb="banned", member=member, case_id=case_id, reason=reason, extra_right=f"DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}")
            await ctx.send(embed=embed)
        except discord.Forbidden:
    ...
    async def prefix_timeout(self, ctx, member: discord.Member, minutes: int, *, reason="No reason provided"):
        ...
        if minutes <= 0 or minutes > 40320:
            await ctx.send("❌ Duration must be between 1 and 40320 minutes (28 days).", delete_after=5); return
        if not isinstance(ctx.author, discord.Member) or not ctx.guild: return
        if not has_moderation_permissions(ctx.author, member):
            await ctx.send("❌ You don't have permission to timeout this member.", delete_after=5); return
        if not has_higher_role(ctx.guild.me, member):
            await ctx.send("❌ I cannot timeout this member due to role hierarchy.", delete_after=5); return

        until = discord.utils.utcnow() + timedelta(minutes=minutes)
        case_id = self._next_case_id(ctx.guild.id)
        dm_sent = False
        try:
            dm = discord.Embed(title=f"You were timed out in {ctx.guild.name}", description=f"**Duration:** {minutes} minutes\n**Until:** <t:{int(until.timestamp())}:F>\n**Reason:** {reason}", color=discord.Color.yellow(), timestamp=discord.utils.utcnow())
            if ctx.guild.icon: dm.set_thumbnail(url=ctx.guild.icon.url)
            await member.send(embed=dm); dm_sent = True
        except Exception: dm_sent = False
        ...
        try:
            await member.timeout(until, reason=f"Timed out by staff: {reason}")
            self._store_case(ctx.guild.id, case_id, action="Timeout", target_id=member.id, target_tag=f"{member}", moderator_id=ctx.author.id, reason=reason, extra={"dm_sent": dm_sent, "minutes": minutes, "until": int(until.timestamp())})
            e = self._dyno_line(guild=ctx.guild, verb="timed out", member=member, case_id=case_id, reason=reason, extra_right=f"Duration: {minutes}m • DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}")
    ...
    async def prefix_untimeout(self, ctx, member: discord.Member, *, reason="No reason provided"):
        ...
        if not isinstance(ctx.author, discord.Member) or not ctx.guild: return
        if not has_moderation_permissions(ctx.author, member):
            await ctx.send("❌ You don't have permission to remove timeout from this member.", delete_after=5); return

        case_id = self._next_case_id(ctx.guild.id)
        dm_sent = False
        try:
            dm = discord.Embed(title=f"Your timeout was removed in {ctx.guild.name}", description=f"**Reason:** {reason}", color=discord.Color.green(), timestamp=discord.utils.utcnow())
            if ctx.guild.icon: dm.set_thumbnail(url=ctx.guild.icon.url)
            await member.send(embed=dm); dm_sent = True
        except Exception: dm_sent = False
        ...
        try:
            await member.timeout(None, reason=f"Timeout removed by staff: {reason}")
            self._store_case(ctx.guild.id, case_id, action="Untimeout", target_id=member.id, target_tag=f"{member}", moderator_id=ctx.author.id, reason=reason, extra={"dm_sent": dm_sent})
            e = self._dyno_line(guild=ctx.guild, verb="untimeout(ed)", member=member, case_id=case_id, reason=reason, extra_right=f"DM Sent: {'Yes ✅' if dm_sent else 'No ❌'}")
            await ctx.send(embed=e)
        except discord.Forbidden:
    ...
    async def prefix_unban(self, ctx, user_id: int, *, reason: str = "No reason provided"):
        ...
        if not ctx.author.guild_permissions.ban_members:
            await ctx.send("❌ You don't have permission to unban members.", delete_after=5); return
        try:
            user = await self.bot.fetch_user(user_id)
            try:
                await ctx.guild.fetch_ban(user)
            except discord.NotFound:
                await ctx.send("❌ That user is not banned from this server.", delete_after=5); return

            case_id = self._next_case_id(ctx.guild.id)
            await ctx.guild.unban(user, reason=f"Unbanned by staff: {reason}")
            self._store_case(ctx.guild.id, case_id, action="Unban", target_id=user.id, target_tag=f"{user}", moderator_id=ctx.author.id, reason=reason, extra={})

            embed = self._dyno_line(guild=ctx.guild, verb="unbanned", member=user, case_id=case_id, reason=reason)
            await ctx.send(embed=embed)
